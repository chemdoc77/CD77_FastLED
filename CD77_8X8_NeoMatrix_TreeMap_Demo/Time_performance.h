...
void Performance()
{
  ...
  FROM(0,0,16.000)   {CD77_rotatingPalette();gCurrentPalette= CD77_Bgold_p ;  FastLED.show();}
  FROM(0,0,24.000)   {fallingPalette(); gCurrentPalette= Magenta_Evening_gp; FastLED.show();} 
  FROM(0,0,32.010)   {speed = 50; rotatingRainbow();FastLED.show();} 
  FROM(0,0,40.000)   {westwardRainbow();FastLED.show();}
  FROM(0,0,48.000)   {northwardRainbow(); FastLED.show();}
  FROM(0,0,56.000)   {CD77_northeastwardRainbow_15(); FastLED.show();}
  FROM(0,1,04.000)   {speed= 30; CD77_rotatingPalette(); gCurrentPalette= CD77_RYGB_p; FastLED.show();}  
  FROM(0,1,12.000)   {speed= 30; rotatingPalette(); gCurrentPalette= jet_gp; FastLED.show();}  
