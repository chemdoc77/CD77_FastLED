...
void Performance()
{
  ...
  FROM(0,0,50.000)   {cd77_police_lights_one(2, 10, 50);}
  FROM(0,0,55.000)   {cd77_police_lights_all(2, 10, 50);}
  FROM(0,1,00.000)   {cd77_police_lights_one_palette(1,48,6,50);currentPalette = PartyColors_p;  currentBlending = LINEARBLEND;} //note: cd77_police_lights_all_palette( colorIndex,  deltacolorIndex, number of segments,  wait )
  FROM(0,1,05.000)   {cd77_police_lights_all_palette(1,48,6,50);currentPalette = PartyColors_p;  currentBlending = LINEARBLEND;} //note: cd77_police_lights_one_palette( colorIndex,  deltacolorIndex, number of segments,  wait )
  AT(0,1,10.000)     {RestartPerformance(); } 
}
#endif
