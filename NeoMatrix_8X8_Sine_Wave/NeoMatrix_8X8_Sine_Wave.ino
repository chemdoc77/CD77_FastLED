// Sine wave sketch for 8X8 NeoMatrix by Chemdoc77

// The inspiration for the sinewave code functions and the starting point for sinewave code functions are from posts by Sean Clohesy on FastLED G+ Community 
...
void Performance()
{
  FROM(0,0,0.01)     { cd77_matrix_sinewave(3, 0, 60, 7); }
  FROM(0,0,7.00)     { cd77_matrix_sinewave_beatsin(3, 20,  0,  160, 7);  }
  FROM(0,0,14.000)   { cd77_matrix_sinewave_palette(3, 0, 24, 7); currentPalette = RainbowColors_p; currentBlending = NOBLEND;}
  FROM(0,0,21.000)   { cd77_matrix_sinewave_palette(3, 0, 24, 7); currentPalette = test_PaletteRWB3_p;  currentBlending = NOBLEND;}
  //rotate sawtooth wave 90 degrees
  FROM(0,0,28.000)   { cd77_matrix_sinewave_r90(3, 0, 60, 7);}
  FROM(0,0,35.000)   { cd77_matrix_sinewave_beatsin_r90(3, 20,  0,  160, 7);}
  FROM(0,0,42.000)   { cd77_matrix_sinewave_palette_r90(3, 0, 24, 7); currentPalette = RainbowColors_p;  currentBlending = NOBLEND;}
  FROM(0,0,49.000)   { cd77_matrix_sinewave_palette_r90(3, 0, 24, 7); currentPalette = test_PaletteRWB3_p; currentBlending = NOBLEND;}
...
void cd77_matrix_sinewave(uint16_t z, uint16_t hue, uint16_t deltahue, uint8_t wait ){
  ...
  leds[(NUM_LEDS-1)-((sin8(x+(width*10*z))/32)*matrixwidth+width)] = CHSV(hue,255,255);
  hue+=deltahue;
...
void cd77_matrix_sinewave_palette(uint16_t z, uint8_t colorIndex, uint8_t deltacolorIndex, uint8_t wait ){
  ...
  leds[(NUM_LEDS-1)-((sin8(x+(width*10*z))/32)*matrixwidth+width)] = ColorFromPalette( currentPalette, colorIndex+sin8(x), 255, currentBlending);
...
void cd77_matrix_sinewave_r90(uint16_t z, uint8_t hue, uint8_t deltahue, uint8_t wait ){
  ...
  fadeToBlackBy( leds, NUM_LEDS, 150);
  for (uint16_t width =0; width<matrixwidth; width++){
  leds[array1[(NUM_LEDS-1)-((sin8(x+(width*10*z))/32)*matrixwidth+width)]] = CHSV(hue,255,255);
  hue+=deltahue;
...
void cd77_matrix_sinewave_palette_r90(uint16_t z, uint8_t colorIndex, uint8_t deltacolorIndex, uint8_t wait ){
  ...
  leds[array1[(NUM_LEDS-1)-((sin8(x+(width*10*z))/32)*matrixwidth+width)]] = ColorFromPalette( currentPalette, colorIndex+sin8(x), 255, currentBlending);
